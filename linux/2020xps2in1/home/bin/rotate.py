...
# Lightly modified from https://gist.githubusercontent.com/ei-grad/4d9d23b1463a99d24a8d/raw/rotate.py :
# * explicitly use python2
# * use xrandr `--rotate` on a specific output, rather than older `--orientation`
...
def rotate(state):
    s = STATES[state]
    check_call(['xrandr', '--output', output, '--rotate', s['rot']])
